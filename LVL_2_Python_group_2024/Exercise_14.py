# Skeleton program code for Stage 1 Summer 2023 marathon exam
# this code should be used for educational purposes only
# written by senjor .net developer of LokMartin company Harmash D.V.
# developed in the Python 3.11 programming environment (supported in versions below version 3.61)
...
class Breakthrough():
    def __init__(self):
        ...
        self.__CountCardOfDeck = CardCollection("COUNT CARD OF DECK")
    ...
    def GetRemainingCards(self):
        return self.__Deck.GetNumberOfCards()
        ...
        if len(self.__Locks) > 0:
            ...
            while not self.__GameOver:
                ...
                while not self.__LockSolved and not self.__GameOver:
                    ...
                    print(self.__CountCardOfDeck.cards_remaining_in_deck())
                    ...
                    if self.__CurrentLock.GetLockSolved():
                        ...
                        print(f"Cards remaining in deck: {self.GetRemainingCards()}")
    ...
    def __GetRandomLock(self):
        return self.__Locks[randint(0, len(self.__Locks) - 1)]
    ...
    def __MoveCard(self, FromCollection, ToCollection, CardNumber):
        Score = 0
        if FromCollection.name == "HAND" and ToCollection.name == "SEQUENCE":
...
class Card():
    ...
    def __init__(self):
        self.cardNumber = Card._NextCardNumber
    ...
    def cardNumber(self, num):
        self.__cardNumber = num if num else 0
...
class CardCollection():
    ...
    @name.setter
    def name(self, n):
        self.__name = n if n else 'Erorr name'

    def GetCardNumberAt(self, X):
        return self._Cards[X].cardNumber

    def GetCardDescriptionAt(self, X):
        return self._Cards[X].cardDescription
    ...
    def RemoveCard(self, CardNumber):
        ...
        while Pos < len(self._Cards) and not CardFound:
            if self._Cards[Pos].cardNumber == CardNumber:
    ...
    def GetCardDisplay(self):
        ...
        Pos = 0
        while not Complete:
            CardDisplay += "| " + self._Cards[Pos].cardDescription + " "
            Pos += 1
